...
func s3ml(r *http.Request, h http.Header, b *bytes.Buffer) *weft.Result {
	...
	case "POST":
		...
		if err := unmarshal(by, &e); err != nil {
			return weft.InternalServerError(err)
		}

		log.Printf("received %s", e.PublicID)

		if err := e.save(); err != nil {
			return weft.InternalServerError(fmt.Errorf("saving %s: %s", e.PublicID, err.Error()))
		}

		log.Printf("saved %s", e.PublicID)
...
func unmarshal(seisComPML []byte, e *event) error {
	...
	if err = sc3ml.Unmarshal(seisComPML, &s); err != nil {
		return fmt.Errorf("unmarshalling SeisComPML: %s", err.Error())
	}

	if len(s.EventParameters.Events) != 1 {
		return fmt.Errorf("expected 1 event, got %d", len(s.EventParameters.Events))
	}
