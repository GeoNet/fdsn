...
func parseStationV1(v url.Values) (fdsnStationV1Search, error) {
	...
	p := fdsnStationV1Parm{
		...
		StartTime:         Time{zeroDateTime},
		EndTime:           Time{emptyDateTime},
	}
...
func (v fdsnStationV1Search) validStartEnd(start, end time.Time, level int) bool {
	...
	if !end.Equal(zeroDateTime) && v.StartTime.Time.After(end) {
		return false
	}

	if !start.Equal(zeroDateTime) && v.EndTime.Time.Before(start) {
		return false
	}
	return true
}
